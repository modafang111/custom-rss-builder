...
// クライアント: 無料＋利用可のとき有料プラン申込を表示。サーバー開発用 ol は正本のみ。
...
$slot_range_label       = function_exists( 'crb_license_format_slot_range_text' )
	...
	: (string) $max_slots;
$plan_label             = function_exists( 'crb_license_plan_label_for_state' ) ? crb_license_plan_label_for_state( $state ) : ( function_exists( 'crb_license_plan_label' ) ? crb_license_plan_label( $state['plan'] ) : $state['plan'] );
...
<div class="wrap crb-admin-wrap">
	...
	<div class="crb-panel crb-panel--registration-cta">
		...
		<p>
			...
			<?php if ( '' !== $standard_subscribe_url ) : ?>
				<a class="button" href="<?php echo esc_url( $standard_subscribe_url ); ?>" target="_blank" rel="noopener noreferrer">
	...
	<div class="crb-panel">
		...
		<table class="widefat striped crb-license-plan-table crb-license-plan-table--three-col">
			<thead>
				<tr>
	...
	<?php if ( $show_client_standard_cta || $show_client_pro_cta ) : ?>
	<div class="crb-panel crb-panel--paid-plans-cta">
		<h2 class="crb-panel__title"><?php esc_html_e( '有料プランのご案内', 'custom-rss-builder' ); ?></h2>
		<?php if ( $show_client_standard_cta ) : ?>
		<div class="crb-paid-plan-cta crb-paid-plan-cta--standard">
			<h3><?php esc_html_e( 'スタンダード', 'custom-rss-builder' ); ?></h3>
			<p>
				<?php
				if ( function_exists( 'crb_license_standard_plan_summary' ) ) {
					echo esc_html( crb_license_standard_plan_summary() );
				}
				if ( function_exists( 'crb_standard_monthly_price_label' ) ) {
					echo ' · ' . esc_html( crb_standard_monthly_price_label() );
				}
				?>
			</p>
			<p>
		...
		<?php endif; ?>
		<?php if ( $show_client_pro_cta ) : ?>
		<div class="crb-paid-plan-cta crb-paid-plan-cta--pro">
			<h3><?php esc_html_e( 'Pro', 'custom-rss-builder' ); ?></h3>
			<p>
				<?php
				if ( function_exists( 'crb_license_pro_plan_summary' ) ) {
					echo esc_html( crb_license_pro_plan_summary() );
				}
				if ( function_exists( 'crb_pro_monthly_price_label' ) ) {
					echo ' · ' . esc_html( crb_pro_monthly_price_label() );
				}
				?>
			</p>
			<p>
		...
		<?php endif; ?>
		<p class="description">
			<?php esc_html_e( 'お支払い後、ライセンスキーをメールでお送りします。届いたキーを下のフォームに入力して「有効化」してください。', 'custom-rss-builder' ); ?>
		</p>
		<p class="description">
			<?php esc_html_e( '有料プラン特典: 最初の 1 フィードの初期設定代行が 1 回無料です（2 回目以降 1,100 円（税込）／回）。', 'custom-rss-builder' ); ?>
	...
	<div class="crb-panel crb-panel--pro-setup">
		<h2 class="crb-panel__title"><?php esc_html_e( '初期設定代行（有料プラン）', 'custom-rss-builder' ); ?></h2>
		<p>
