		...
		for ( $item_index = 0; $item_index < $scan_limit; $item_index++ ) {
			...
			if ( false !== $text_nodes ) {
				for ( $i = 0; $i < $text_nodes->length && count( $candidates ) < 80; $i++ ) {
					...
					$this->add_item_inner_candidate( $candidates, $seen, 'text', 'text', $this->suggest_text_selector( $node, $item, strtolower( $node->tagName ) ), min( 40, mb_strlen( $text ) ), count( $seen ) );
				}
			}
		}
